...
next80 can create relocatable files that conform to the format generated by the SDAS assembler and interpreted by the SDLS linker. These tools are used internally by [the SDCC compiler](https://sdcc.sourceforge.net/), thus next80 can be used to build relocatable files that can be be combined with C code to produce a full executable program (there's an example of this at the end of this document).

["Writing relocatable code"](WritingRelocatableCode.md) explains generic concepts about what "relocatable code" is and how linkers convert it into full programs. This document contains specific information regarding the SDCC file format and how next80 supports it.
...
next80 supports generating SDCC relocatable files conforming to both the "XL3" and "XL4" format specifiers (hexadecimal numbers, low-endian for multi-byte values, 24-bit addressing for byte values). By default, next80 generates the modern "XL4" format which matches the relocatable files generated by SDAS in SDCC 4.5.0 and later. You can explicitly request a specific format by using `--build-type sdcc-xl3` or `--build-type sdcc-xl4`.

The file encoding of the SDCC relocatable files generated by next80 is UTF-8 without byte order mark (this is required by SDAS). Lines are terminated with the system default end of line sequence unless a different one is specified by passing an `--end-of-line` argument to next80.
...
* [`DEFS`](LanguageReference.md#defs-ds): the `<value>` argument can't be used, and the next80 argument `--initialize-defs` won't have any effect.
...
See ["Expressions"](LanguageReference.md#expressions) for more details about the next80 support for expressions in assembler code.


## A simple example

Here we'll see an example of how to generate a standard binary file for [MSX](https://en.wikipedia.org/wiki/MSX) computers using next80 and [SDCC](https://sdcc.sourceforge.net/) 4.0 or newer. These files can be loaded from the [MSX-BASIC](https://github.com/Konamiman/MSX2-Technical-Handbook/blob/master/md/Chapter2.md) environment using the `BLOAD"file"` command, or loaded and executed with the `BLOAD"file",R` command.
...
This is the command to assemble this file with next80, the resulting file will be `crt0_msx_basic.REL`:

```
n80 crt0_msx_basic.asm
```

next80 will automatically set the build type to SDCC relocatable thanks to `area _HEADER (ABS)` being the first line of the code, but you could also have added `--built-type sdcc` to the next80 command line.
...
The important concept to learn here is that you can use next80 to create reusable relocatable files (like the `print.REL` in the example) that you can integrate with your C programs. This is good because next80 has powerful features that aren't available in SDAS (like phased blocks, custom error messages, proper character encodings for strings, modules and relative labels) and uses a familiar syntax for Z80 code, e.g. `(IX+n)` instead of SDAS' `n(IX)`.
