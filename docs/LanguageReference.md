# next80 assembler language reference

This document details the source file format supported by the **n80** assembler (part of the next80 toolchain) and lists all available assembler instructions (called "pseudo-operators" in the MACRO-80 manual), both those inherited from MACRO-80 and those newly introduced. It also explains basic concepts about how n80 works (e.g. passes, absolute vs relocatable assembly) and details the available advanced features (e.g. conditional assembly, macros, symbol scoping).

The next80 toolchain consists of three tools: **n80** (assembler), **lk80** (linker), and **lb80** (library manager). The last two are only relevant when [writing relocatable code](WritingRelocatableCode.md). This document discusses the assembler only.

n80 is almost fully compatible with MACRO-80. You may also want to take a look at [the original MACRO-80 user manual](MACRO-80.txt) for things that might be missing here or that are explained differently.
...
🆕 A "new" icon is used when introducing a feature or instruction that is new in next80 (it wasn't available in MACRO-80).

✨ A "sparks" icon is used when referring to a feature or instruction that was already available in MACRO-80 but has been enhanced or improved in a backwards-compatible way in next80.

🚫 A "forbidden" icon is used to refer to a MACRO-80 feature or instruction that is not available or has changed in a backwards-incompatible way in next80. Old source code intended for MACRO-80 and relying in such features or instructions will likely require changes before being suitable for assembly with next80.
...
next80 treats assembler instructions and CPU instructions in a case-insensitive way. In the examples in this document instructions are written in lower case, while symbols and macro names are written un upper case.

This documents refers to some (but not all) of the available next80 command line arguments. To get the full reference of the existing arguments you can run next80 with the `--help` argument, or you can look at [the help text in the next80 source code](https://github.com/Konamiman/next80/blob/master/n80/Program.Help.cs).
...
next80 is a _two pass assembler_. This means that the source code is processed twice: the first time the assembler determines how many bytes each source code line will use in the _target program_ (the contents of the binary file resulting from processing the source code) and updates the current _location counter_ (the pointer in Z80 memory where the code generated by each line will be loaded) accordingly, using this information to assign the appropriate values to [labels](#source-code-format). The second time, already with all the label values at hand, is when the code is actually assembled.

⚠ When the location counter overflows (its value goes beyond FFFFh) after an instruction that generates output (so not including [`ORG`](#org)), it will go back to 0 and a warning will be generated.
...
next80 can produce both absolute 🆕 and relocatable files.
...
💡 If you want to know the exact source line in which next80 selects the build type, run next80 with a verbosity level of at least two (with the `--verbosity` argument).
...
⚠ Pass the `--link-80-compatibility` argument to next80 if you are building a MACRO-80 relocatable file and you plan to use the old LINK-80 tool instead of lk80 for the linking process. This argument forces next80 to generate relocatable files using the old MACRO-80/LINK-80 format instead of the new extended format. See ["Relocatable file format"](RelocatableFileFormat.md) for details on the differences between both formats.
...
When producing absolute output there are two strategies that next80 can use:
...
"Memory map" is the default strategy, the `--direct-output-write` argument can be used to instruct next80 to use the "Direct output file write" strategy instead.
...
MACRO-80 assumes that the source code is encoded in ASCII, next80 however supports source code supplied in any character encoding. ✨

MACRO-80 treats source code characters with the high bit set as line numbers generated by a text editor, this is not the case of next80. 🚫
...
next80 treats symbols in a case-insensitive way, e.g. `foo`, `FOO` and `Foo` refer all to the same symbol.

There's in principle no limit for the length of a symbol ✨, but when assembling MACRO-80 relocatable code with the `--link-80-compatibility` argument the maximum length for external and public symbols will be 6 characters. This limitation is given by [the format of the LINK-80 relocatable files](RelocatableFileFormat.md). next80 will issue a warning if it finds two or more external symbols that are different but share the first 6 characters (since these will get their names truncated and thus be actually the same symbol), and will throw an error if the same happens with public symbols.
...
💡 If the `--unknown-symbols-external` argument is passed to next80 then any symbol that is still unknown at the end of pass 2 will be implicitly considered an external symbol.
...
A _fixed constant_ is defined using the following syntax: `<name>[:] EQU <value expression>`. The colon after the constant name is optional ✨ (in MACRO-80 the colon after the constant name isn't allowed). Example:
...
A _redefinible constant_ is (re)defined using the following syntax: `<name>[:] DEFL <value expression>`. As in the case of the fixed constants, the colon after the constant name is optional ✨. Example:
...
⚠ (🚫?) The MACRO-80 manual mentions also the instruction `SET` as an additional alias for [`DEFL`](#defl-aset), but in practice it doesn't work as such: `SET` is only recognized as the Z80 instruction of the same name in MACRO-80. next80 behaves as MACRO-80 behaves and not as the MACRO-80 documentation says, so you can't use `SET` to define constants.
...
⚠ The <code>B</code> and <code>D</code> suffixes are actually unusable when the default radix is 12 or higher and 14 or higher, respectively. Consider the following example:
  ...
  That's the reason why the new suffixes `I` (b**I**nary) and `M` (deci**M**al) have been introduced in next80.
</blockquote>

💡 The `#` prefix for hexadecimal numbers has been introduced in next80 for compatibility with other assemblers that use the same notation, but in general it's recommended to use the `H` suffix instead since `#` could cause confusion with the suffix for external symbols, `##`.
...
The _current character encoding_ is the one specified in a `--string-encoding` argument when running next80, and can also be changed in code with the [`.STRENC`](#strenc-) instruction. You can run next80 with the `--list-encodings` argument to see a list of available encodings. The default encoding when none is specified is 7 bit ASCII.
...
The support for escape sequences in double-quoted strings can be disabled by using the next80 command line argument `--no-string-escapes` or the instruction `.STRESC OFF` in code. When escape sequences are disabled the double quote character itself can still be escaped by doubling it:
...
⚠ Escape sequences are always processed as such in messages enclosed in double quotes for [`.PRINT`](#print-), [`.WARN`](#warn-), [`.ERROR`](#error-) and [`.FATAL`](#fatal-) instructions, even if escape sequences for regular strings are disabled with `.STRESC OFF` or by passing the `--no-string-escapes` argument to next80.
...
next80 defines the following arithmetic operators:
...
💡 The [`IFB`](#ifb) and [`IFNB`](#ifnb) instructions can be used alternatively to the `NUL` operator in order to check for empty arguments in [macros](#macros).
...
⚠🚫 The `TYPE` operator is intended to be used with plain symbols and will act weirdly with complex expressions. For example, `TYPE (FOO##+1)` will evaluate to A0h in MACRO-80 and throw an error in next80.
...
In next80 bare expressions aren't supported by default 🚫, but they will be supported if the `--allow-bare-expressions` command line argument is used. You might need this to assemble old source code, but in general bare expressions shouldn't be used since they can cause confussion (for example if you intend to introduce a named macro expansion and mistype the macro name you'll get a confusing "symbol not found" error).


## Symbol scoping

next80 offers two mechanism for scoping symbols so that the same symbol names can be reused in different parts of the same source code: modules and relative labels.
...
If you are using modules in a program with external symbol declarations you can instruct next80 to implicitly consider all external symbols as root symbols, see [`.EXTROOT`](#extroot-).
...
The following IF instructions are supported by next80, see the reference of each instruction for the syntax details:
...
⚠ A new macro definition will replace one that already exists with the same name. This behavior is compatible with MACRO-80, but in next80 it will emit a warning. Example:
...
next80 allows to generate a listing with information about the generated addresses and symbols, displayed together with the source code. A listing is generated if the `--listing-file` argument is supplied to next80.
...
```
<form feed character><title> next80 <version> PAGE <page>[-<subpage>]
<subtitle>
...
* `<version>` is the version number of the next80 program that generated the listing.
...
By default the `<bytes>` area will only display four bytes, and any remaining bytes (e.g. for long [`DEFB`](#defb-db-defm) or [`INCBIN`](#incbin-) instructions) will appear below in extra lines, without source line. The amount of bytes per line can be configured 🆕 with the next80 argument `--listing-bytes-per-line`.

By default a total of up to 128 bytes will be displayed for one single instruction in the `<bytes>` area; if an instruction generates more bytes, the excess bytes will be left out of the listing and "..." will be added after the last printed byte. This limit can be configured 🆕 with the next80 argument `--listing-max-bytes-per-instruction`.
...
By default four symbols will be listed in each listing line, and only the first 16 characters of each symbol will be printed (longer symbols will be truncated and will get a `...` at the end). These values can be configured 🆕 with the next80 arguments `--listing-symbols-per-line` and `--listing-max-symbol-length`, respectively.

By default all symbols, macro names and area names in the symbols list will keep their original casing 🆕, but next80 can be instructed to convert them to uppercase (which is what MACRO-80 does when generating listings) with the  `--listing-uppercase-symbols` argument.
...
* The `--no-listing-false-conditionals` next80 argument will set the initial state to "suppress false conditional blocks in the listing".
* The [`.LFCOND`](#lfcond) instruction will insruct next80 to include false conditional blocks in listings.
* The [`.SFCOND`](#sfcond) instruction will insruct next80 to suppress false conditional blocks from listings.
* The [`.TFCOND`](#tfcond) instruction will insruct next80 to toggle (invert) the inclusion or supression of false conditional blocks in listings, **but** based on the initial state or on the state set by the previous [`.TFCOND`](#tfcond) instruction; previous executions of [`.LFCOND`](#lfcond) and [`.SFCOND`](#sfcond) are **not** taken in account.


### Suppressing macro expansions in listings

By default, when next80 is listing a macro expansion it will only include the source lines that generate output (so it won't include comment-only lines or any lines that aren't CPU instructions or [`DEFB`](#defb-db-defm), [`DEFS`](#defs-ds) etc lines). The following instructions are available to configure this behavior:

* [`.LALL`](#lall) instructs next80 to include all macro expansion lines in listings.
* [`.SALL`](#sall) instructs next80 to not include macro expansions at all in listings.
* [`.XALL`](#xall) instructs next80 to go back to the default behavior (list only macro expansion lines that generate output).


### Additional next80 arguments for listings

Additionally to the next80 arguments already mentioned, the following ones are available to configure the listing generation process:
...
This section lists all the assembler instructions supported by next80. Any instruction alias is listed together with the "canonical" instruction name.
...
⚠ This instruction can be used only when buidling an absolute file. Address alignments for relocatable files are controlled with the lk80's `--align-code` and `--align-data` arguments (make sure that you are using lk80 v1.1 or newer). See ["Writing relocatable code"](WritingRelocatableCode.md).
...
In MACRO-80 this instruction enabled the inclusion of cross-reference information when generating a listing file. next80 doesn't implement cross-reference information generation and thus this instruction does nothing.
...
Instructs next80 to consider all the declared external symbols as root symbols, so you don't need to register them with [`ROOT`](#root-) or prepend them with `:` to use them inside a module.
...
Instructs next80 to include all the source code text following the instruction. This is the default initial condition when next80 is instructed to generate a listing file with the `--listing-file` argument. See also [`.XLIST`](#xlist), ["Listings"](#listings).
...
Prints a text to the terminal where the assembler is running (unless the `--silence-assembly-print` argument was passed to next80). The message supports [expression interpolation](#expression-interpolation-).
...
Instructs next80 to **not** include conditional blocks that evaluate as false in listings for all the matching blocks following the instruction. See also [`.LFCOND`](#lfcond), [`.TFCOND`](#tfcond), "Conditional blocks", ["Listings"](#listings).
...
The encoding name or page must be one of the encodings supported by the system where next80 is running. Running next80 with the `--list-encodings` argument will show a list of the available encodings; each encoding has an unique name and an unique page number and either of the two can be used to identify the encoding. Encoding names are case-insensitive.

The default encoding is 7 bit ASCII unless a different encoding is specified by passing a `--string-encoding` argument to next80. `.STRENC default` will revert to this default encoding.
...
Turns on or off the support for escape sequences in strings delimited by double quotes, `"`. Escape sequences are enabled by default unless a `--no-string-escapes` argument is passed to next80.
...
Instructs next80 to toggle the inclusion of conditional blocks that evaluate as false in listings, from on to off or the other way around, for all the matching blocks following the instruction. 

The state that gets toggled is the one that was set by the previous instance of `.TFCOND` (any state change performed with [`.LFCOND`](#lfcond) or [`.SFCOND`](#sfcond) is ignored). The initial state is "on" unless a `--no-listing-false-conditionals` argument is passed to next80.
...
Instructs next80 to include macro expansions in listings following the instruction, but only including the source lines that produce any output in the target file (assembler instructions and [`DEFB`](#defb-db-defm), [`DEFW`](#defw-dw) etc); this is the default state for macro expansions in listings. See [`.SALL`](#sall), [`.LALL`](#lall), ["Macros"](#macros), ["Listings"](#listings).
...
In MACRO-80 this instruction disabled the inclusion of cross-reference information when generating a listing file (which had been enabled with [`.CREF`](#cref-)). next80 doesn't implement cross-reference information generation and thus this instruction is a no-op.


### .XEXTROOT 🆕

Instructs next80 to no longer consider external symbols as root symbols inside modules. See [`.EXTROOT`](#extroot-).
...
Instructs next80 to suppress the inclusion in listings of the source code text that follows the instruction, until either the end of the file (or an [`END`](#end) instruction) is reached or a [`.LIST`](#list) instruction is encountered. See also ["Listings"](#listings).
...
⚠ The syntax of this command is the same as the command of the same name in the SDAS assembler, however next80 doesn't support paged areas and thus the `PAG` argument isn't available.
...
* When the build type is MACRO-80 relocatable and a `--initialize-defs` argument is supplied to next80, `DEFB <size>` is equivalent to `DEFB <size>,0`.
...
Instructs next80 to stop generating output immediately while continuing to process the source code that follows the instruction (thus taking note of symbol definitions). This is useful as an alternative to constant definitions to define memory areas that don't need to be explicitly included in the output file.
...
Additionally, if the `--unknown-symbols-external` argument is passed to next80 then any symbol that is still unknown at the end of pass 2 will be implicitly considered an external symbol.
...
⚠ You may be wondering why the previous example doesn't use `if (x) gt 0` or `if (x) lt 0` for the positive and negative cases. next80 treats all numbers as unsigned integers, and thus `if (x) lt 0` is never true. This behavior is compatible with MACRO-80.
...
Starts a conditional assembly block in which the true condition is that `<symbol>` is defined when the instruction is encountered. This may be useful in combination with the `--define-symbols` next80 argument to externally alter how the code is assembled. The opposite instruction is [`IFNDEF`](#ifndef).
...
The file include mechanism in next80 implements two important enhancement compared to MACRO-80:

1. `<file path>` accepts full and relative path specifications (with directory and drive -if supported by the operating system where next80 is running- specifications), not just plain file names.
...
1. The current directory (in the terminal where next80 was run).
2. The directory of the source file where the `INCLUDE` instruction was found.
3. Any extra directories passed to next80 with `--include-directory`, in the order in which they were specified.

For example, if next80 is run like this:
...
...then next80 will search for `graphics.asm` using the following full paths, in that order:
...
The `SUBPAGE` alias is introduced in next80 because the word "PAGE" doesn't clearly convey the fact that what is changing is the _sub_page number. It's recommended to combine it with the new [`MAINPAGE`](#mainpage-) instruction if both main page changes and sub page changes are required for listings.
...
Sets the title to be used in the heading of each page of a listing, as the very first line of text (together with the next80 version number).
