...
This document details the format of the LINK-80 relocatable files that next80 generates when the build type of the generated file is "relocatable". The information here comes mainly from the original MACRO-80 manual, but extra information that was missing in the original manual has been added after doing some reverse engineering of `.REL` files generated by MACRO-80.

lk80, the linking loader that is part of the next80 project, is backwards-compatible with the old LINK-80 tool that was bundled together with MACRO-80: lk80 defines an _extended relocatable file format_ that is a superset of the original relocatable file format used by MACRO-80 and LINK-80. This document details the original relocatable file format first, and then [the additions introduced in the extended file format](#extended-relocatable-file-format).
...
This is an extension mechanism that allows to define additional link items, the items generated by MACRO-80 and next80 are listed in ["Extension link items"](#extension-link-items).
...
This link item is never generated by MACRO-80 nor by next80.
...
This item is similar to "Chain external" but is supposed to be used for relocatable addresses instead of external symbol references. However this item is never generated by MACRO-80 or next80.
...
The following extension link items are generated by MACRO-80 and next80, they are used to represent expressions that need to be evaluated at link time (this happens for two reasons: expressions that contain external symbol references, and non-absolute values that need to be stored as one single byte).
...
Note that not all the existing operators are in the list (e.g. AND, OR... are missing). The [extended relocatable file format](#extended-relocatable-file-format) defines [additional codes](#additional-arithmetic-operators) for the missing operators, these are supported by lk80 but not by LINK-80. Trying to use the additional operators in an expression with external symbol references will result in an error at assembly time when using MACRO-80, and also when using next80 with the `--link80-compatibility` argument.
...
The extended relocatable file format defined for usage with next80 and lk80 is the same as the old relocatable file format used by MACRO-80 and LINK-80, with the following additions. These additions are backwards-compatible, meaning that old relocatable files created with MACRO-80 can be processed with lk80.
...
The header is defined in this way so that it will be identified as an empty program by LINK-80 in case this tool is mistakenly used instead of lk80 to process the file.
...
Additionally, the casing of all symbols is preserved (in MACRO-80 compatibility mode they get uppercased), although symbols are still considered as case-insensitive when comparing for equality both in next80 and in lk80.
